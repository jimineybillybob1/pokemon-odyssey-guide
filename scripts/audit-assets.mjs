...
const creditsPath=path.join(root,'sources/asset-credits.json');
const sourceBackgrounds=new Set((fs.existsSync(creditsPath)?JSON.parse(fs.readFileSync(creditsPath,'utf8')):[]).filter(x=>x.preserveSourceBackground).map(x=>x.path));
const unexpectedOpaque=opaquePokemonPngs.filter(file=>!sourceBackgrounds.has(file));
if(opaquePokemonPngs.length-unexpectedOpaque.length)console.warn(`WARN: ${opaquePokemonPngs.length-unexpectedOpaque.length} credited workbook sprites preserve their original coloured backgrounds.`);
if (unexpectedOpaque.length) {
  unexpectedOpaque.forEach(file => console.error(`OPAQUE POKEMON SPRITE: ${file}`));
  process.exit(1);
}
