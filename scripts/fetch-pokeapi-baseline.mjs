...
// Fetch only source-supported identities when the baseline is an allowlisted fallback.
if (config.pokemonScope === 'hack-only') speciesRefs = speciesRefs.filter(ref => includedPokemonKeys.has(ref.name));
...
for (const entry of species) {
  ...
  for (const variety of varieties) if(config.pokemonScope !== 'hack-only' || includedPokemonKeys.has(variety.pokemon.name)) varietyRefs.push({ species: entry, variety });
}
